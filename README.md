...
2. # Неправильная обработка ошибок:
  - **Строки с уязвимостью:**   Все места, где происходит **log.Fatal(err)**, приводящие к прекращению работы программы.
- **ПОСЛЕДСТВИЯ:**
...
- **Строка с уязвимостью:** **if r.Method != "GET"**.
 - **ПОСЛЕДСТВИЯ**:
   - Менее читаемый и не совсем безопасный код.
  - **ИСПРАВЛЕНИЕ:** 
...
```
# Внесенные изменения:
1. Использовал параметризованный запрос для предотвращения атак **SQL инъекций.**
2. **Удалил чувствительную информацию (имя пользователя и пароль)** из кода и использовал их как локальные переменные в функции **initDB().**
3. Использовал **http.MethodGet** для проверки метода запроса.
4. Использовал оператор **% в запросе SQL** вместо форматирования строки для предотвращения атак SQL инъекций.
...
# Способ №2(лучший на мой взгляд):
 ...
 Добавлю проверку пользовательского ввода переменных ***"name"*** и ***"age"*** для строк:
- ***name = request.values.get('name')***
- ***age = request.values.get('age', 'unknown')***
...
- Использовать специализированные библиотеки или функции для выполнения ***DNS-запросов***, а не выполнять команды через оболочку.

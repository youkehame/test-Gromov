...
- Использование ***шаблонизаторов***, они автоматически обработают(экранируют) входные данные, в нашем случае ***Jinja2***.
 # Способ №1:
- Использую способ ***шаблонизаторов***, в данном коде это ***Jinja2***.
...
# Аргументация
Способ исправления уязвимости через использование шаблонизатора ***Jinja2*** заключается в том, что данный способ обеспечивает безопасное форматирование строк и автоматическое экранирование входных данных, что предотвращает возможность инъекции кода и защищает от атак на основе инъекции шаблонов. Кроме того, использование шаблонизатора позволяет упростить процесс форматирования строк и сделать код более читаемым и понятным.



# Способ №2(лучший на мой взгляд):
...
- Появится возможность получить доступ к личным(секретным) данным, выполнить нежелательные операции или повредить систему.
 ...
 # Способы исправления уязвимости:
- Использовать безопасные методы для выполнения команд, таких как ***subprocess.run()*** с указанием параметров команды.
- Валидация и фильтрация входных данных, это поможет нам предотвратить использование вредоносных команд.
- Использовать специализированные библиотеки или функции для выполнения ***DNS-запросов***, а не выыполняем команды через оболочку.
...
# 3. Моделировани угроз
  **Потенциальные проблемы безопасности**
  ...
  - Если база данных будет взломана, то злоумышленник сможет получить доступ ко всем данным пользователей.
   # ***Нету шифрования данных при передаче.***
   Последствия проблемы:
  - Если данные никак не шифруются, то их можно украсть или изменить под себя.
    # ***Слабый урвоень аутентификации и авторизации пользователей.***
   Последствия проблемы:
  - Повышаются шансы взлома данных.
    # Способы исправления уязвимостей:
  - Можно использовать не одну базу данных, а несколько, также можно использовать разграничение прав доступа, там самым разрешать доступ к некоторым данным в БД не всем.
  - Нужно использовать различные методы шифрования данных,так у злоумышленника будет труднее получить доступ к ним.
  - Можно разработать надёжную систему авторизации которая будет ограничивать доступ к некоторым данным.
